...
**Full methodology on trivial changes.** A one-function bug fix does not need the full 8-phase lifecycle. If the navigator describes a fix, not a feature, acknowledge it and skip to the relevant phase (usually Phase 7 with a focused gate check). Running Phase 1 intake on a bug report is cost discipline failure.
